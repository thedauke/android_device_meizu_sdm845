...
#define LOG_TAG "android.hardware.biometrics.fingerprint@2.1-service.meizu_sdm845"
...
Return<int32_t> BiometricsFingerprint::fpNotify(int32_t cmd, int32_t data) {
    if (!strcmp(fp_vendor, "goodix")) goto goodix;
    switch(cmd) {
        case 1:
            cmd = 0x7f16;
            data = 0;
...
Return<void> BiometricsFingerprint::goodixNotify() {
    return Void();
}
