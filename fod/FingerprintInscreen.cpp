...
Return<void> FingerprintInscreen::onPress() {
    ...
    std::thread([this]() {
        ...
        if (mFingerPressed) {
            this->mSteller->stellerNotify(NOTIFY_FINGER_DETECTED, 0);
            this->mGoodixFingerprintDaemon->goodixNotify(NOTIFY_FINGER_DETECTED);
        }
    }).detach();
    return Void();
}
...
Return<void> FingerprintInscreen::onRelease() {
    mFingerPressed = false;
    this->mSteller->stellerNotify(NOTIFY_FINGER_REMOVED, 0);
    this->mGoodixFingerprintDaemon->goodixNotify(NOTIFY_FINGER_REMOVED);
    return Void();
}
...
void FingerprintInscreen::notifyKeyEvent(int value) {
    ...
    if (value) {
        ...
        if (!ret.isOk()) {
            LOG(ERROR) << "notifyKeyEvent: FingerDown() error: " << ret.description();
        }
    } else {
        Return<void> ret = mCallback->onFingerUp();
        if (!ret.isOk()) {
            LOG(ERROR) << "notifyKeyEvent: FingerUp() error: " << ret.description();
        }
    }
}
