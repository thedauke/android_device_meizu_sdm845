...
HELPER="${DEV_ROOT}/tools/extract_utils.sh"
if [ ! -f "${HELPER}" ]; then
...
setup_vendor "${DEVICE_COMMON}" "${VENDOR}" "${DEV_ROOT}" false "${CLEAN_VENDOR}"

extract "${MY_DIR}/proprietary-files-common.txt" "${SRC}" ${KANG} --section "${SECTION}"

if [ -n "${DEVICE}" ]; then
    # Reinitialize the helper for device
    setup_vendor "${DEVICE}" "${VENDOR}" "${DEV_ROOT}" false "${CLEAN_VENDOR}"
